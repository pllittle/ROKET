...
* @cuturi2013sinkhorn, [paper](https://arxiv.org/pdf/1306.0895.pdf)
* @zhang2020review, [paper](https://arxiv.org/pdf/2008.02995.pdf)
* @nath2020statistical, [paper](https://arxiv.org/pdf/2002.03179.pdf)
* @fatras2021unbalanced, [paper](http://proceedings.mlr.press/v139/fatras21a/fatras21a.pdf)
* @chapel2021unbalanced, [paper](https://arxiv.org/pdf/2106.04145.pdf)
...
* $\bf{X}_i$ denotes $p$ baseline covariates to adjust for, and 
* $\bf{K}_i = (K_{i1},\ldots,K_{iN})^\T$, the $i$th column of kernel matrix $\bf{K}$. 
...
For a single candidate distance matrix, transform the distance matrix to a kernel matrix and convert the object into a list, denoted by the object `KK`, code is provided below from the R package, [**MiRKAT**](https://cran.r-project.org/web/packages/MiRKAT/index.html) [@zhao2015testing;@plantinga2017mirkat].
...
* an R list of kernel matrices (`KK`), 
* the residual vector (`RESI`), and 
* user-supplied number of permutations (`nPERMS`) 

to calculate individual kernel p-values as well as the omnibus test. The omnibus test assesses the significance of the minimum p-value kernel. The function `kernTEST` requires `names(RESI)` and dimension names per object as well as row/column names per kernel within `KK` are named for sample order consistency. Sample code is provided below. Setting `verbose = TRUE` allows the user to track the permutation's progress, especially when requesting hundreds of thousands of permutations.
